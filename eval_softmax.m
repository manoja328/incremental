...
imdb.images.fullname = {};
trpos = find(imdb.images.set == 1 );
valpos = find(imdb.images.set == 3 );
imdb.images.fullname(trpos) = strcat('data/ILSVRC2012/train/',imdb.images.folders(trpos),filesep,imdb.images.name(trpos));
imdb.images.fullname(valpos) = strcat('data/ILSVRC2012/val/',imdb.images.folders(valpos),filesep,imdb.images.name(valpos));
...
%     step = min(256, size(imdb.images.data, 4) - nsamp+1);\
    ...
    step = min(256, size(train, 2) - nsamp+1);
    fprintf('%d out of %d to forward \n',nsamp, size(train, 2))
    
    batch = nsamp:1:nsamp+step-1;
    
    images = imdb.images.fullname(batch);

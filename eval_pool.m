...
% fprintf("pos %d\n",length(pos));
train = pos;
% classtr = unique(train.images.labels);
% fprintf("class %d\n",classtr);

imdbnew = {};
imdbnew.folders = imdb.images.folders(train);
imdbnew.name = imdb.images.name(train);
...
%     tic
    ...
    images = strcat('data/ILSVRC2012/train/',imdbnew.folders(batch),filesep,imdbnew.name(batch));
